...
fulldata <- rbind(data,data7,data8,data9,data10,data11,data12,data13,data14,data15,data16,data17,data18)
fulldata[,1] <- gsub("(\\w{3})(\\w{2})(\\w{4})$","\\1-\\2-\\3",fulldata[,1])
fulldata[,1] <- gsub("(\\d{3})(\\d{2})(\\d{3})$","\\1-\\2-\\3",fulldata[,1])
head(fulldata)

fulldata <-as.data.table(fulldata)
colnames(fulldata) <- c("parcel_id","long","lat")
fulldata <- distinct(fulldata, parcel_id, .keep_all = TRUE)
